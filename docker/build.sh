...
set -x
ARGOS_SERVICE_VERSION="1.0.0"
ARGOS_FRONTEND_VERSION="1.0.0"
...
image() {
  ...
  docker build \
    ...
    --build-arg ARGOS_SERVICE_IMAGE=${ARGOS_SERVICE_IMAGE} \
    --build-arg ARGOS_FRONTEND_IMAGE=${ARGOS_FRONTEND_IMAGE} \
    --tag ${build_image} \
  .
}
